...
@click.option('--save', '-s', type=click.Path(dir_okay=False),
...
def plot(model, title, save, adjust, prefix, tropical):
    ...
    if prefix[-3:] == 'age':
        ...
        df2 = pd.wide_to_long(df.assign(loghpd=df.index), ['age'], i='loghpd',
                              j='Age', sep='_').reset_index()\
                .rename(columns=lambda x: x if x != 'age' else vname)
        df2.Age = df2.Age.astype('category')
        sns.lineplot('loghpd', vname, hue='Age', data=df2,
                     linewidth=1.5, palette=palette)
    else:
        df.plot()
    ax = plt.gca()
    ax.set_title(title)
