...
def calc_hist(dst, data):
  ...
  for idx in range(data.shape[0]):
    hist[idx, :] = np.bincount(data[idx, :, :].reshape(dst.width *
                                                       dst.height),
                               carea.reshape(dst.width * dst.height))
  return hist

def doit(cname):
  ne_fname = '/data/natural-earth/ne_10m_admin_0_countries/' \
    'ne_10m_admin_0_countries.shp'
  esa_lc_fname = '/data/esacci-lc/scratch/' \
    'ESACCI-LC-L4-LCCS-Map-300m-P1Y-1992_2015-v2.0.7.tif'
  esa_lc_legend_fname = '/data/esacci-lc/scratch/' \
    'ESACCI-LC-Legend.csv'
  print('Finding country shape')
  with fiona.open(ne_fname) as ne:
    features = [feat["geometry"] for feat in ne
                if feat['properties']['NAME'] == cname ]
  ...
  legend_df = pd.read_csv(esa_lc_legend_fname, sep=';')[['NB_LAB',
                                                         'LCCOwnLabel']]
  df = pd.merge(legend_df, df, left_on='NB_LAB', right_index=True) \
         .drop('NB_LAB', axis=1)
  long_df = pd.melt(df, value_vars=tuple(range(1992, 2015)),
                    id_vars='LCCOwnLabel', value_name='area',
                    var_name='year')
  return long_df
