...
def get_files(dname, what):
    files = groupby(
        map(
            ...
            map(
                ...
                filter(
                    lambda f: re.search(r"-%s-\d+.tif$" % what, f),
    ...
    for scenario, content in files:
        out[scenario] = ["%s-%s-%s.tif" % (s, w, y) for s, w, y in tuple(content)]
    return out
...
def pline(p, df, column, legend=None, color="black", line_width=3):
    ...
    if legend is None:
        legend = column
    p.line(
        "year", "data", source=src, line_width=line_width, legend=legend, color=color
    )
...
def bokeh_plot(dfs, title=""):
    ...
    p.add_tools(
        HoverTool(
            tooltips=[("Year", "@year"), ("Population", "@data"), ("Scenario", "@name")]
        )
    )
...
@click.option(
    "--outdir", type=click.Path(dir_okay=True, file_okay=False), default="/out/luh2"
)
...
def hyde(outdir):
    ...
    df["historical"] = tuple(
        map(lambda y: _one(ds.name, True, years.index(y) + 1), years)
    )
    df.index = years
...
def sps(outdir, raw=False):
    ...
    for scenario in map(lambda n: "ssp%d" % n, range(1, 6)):
        ...
        if raw:
            ...
            ds = rasterio.open("netcdf:" + os.path.join(outdir, "sps.nc:%s" % scenario))
            years = tuple(
                map(lambda idx: int(ds.tags(idx)["NETCDF_DIM_time"]), ds.indexes)
            )
            df[scenario] = tuple(
                map(lambda y: ds.read(years.index(y) + 1, masked=True).sum(), years)
            )
            df.index = years
        dfs[scenario] = df
    return dfs
...
def projected(outdir, what="hpd"):
    ...
    for scenario in files:
        ...
        df = pd.DataFrame()
        df[scenario] = tuple(
            map(lambda f: _one(os.path.join(outdir, f), True, 1), files[scenario])
        )
        df.index = tuple(
            map(
                lambda f: int(os.path.splitext(f)[0].rsplit("-", 1)[-1]),
                files[scenario],
            )
        )
        dfs[scenario] = df
...
def old_school():
    ...
    for name, fname, scale in (
        ("gluds qd", "/out/luh2/gluds00ag.tif", True),
