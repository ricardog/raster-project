...
def do_block(dst, win, conn_pool, template):
    ...
    try:
        ...
        conn_pool.putconn(conn)
    etime = time.time()
    print("[%d:%d] executed in %7.2fs" % (win.col_off, win.row_off, etime - stime))
    if len(data) > 0:
        put(out, data)
...
@click.argument("path", type=click.Path(dir_okay=False), default="road-length.tif")
...
def generate(table, clause, ref_raster, path, num_workers, db, host, user, password):
