...
def do_block(win, mask, index):
  ...
  startx, starty = affine * (win[1][0] - xres / 2.0,
                             win[0][0] - yres / 2.0)
  endx, endy = affine * (win[1][1] - xres / 2.0,
                         win[0][1] - yres / 2.0)
...
def densify(mask, shapes):
  if mask is None:
    mask = 'zip:%s!ICE_1km_2005.bil' % os.path.join(utils.data_root(),
                                                    '1km', 'ICE.zip')
  if shapes is None:
    shapes = os.path.join(utils.data_root(),
  ...
  with fiona.open(shapes) as layer:
    with rasterio.open(mask) as mask_ds:
      ...
      cProfile.runctx('wrap()', filename='restats', locals={'wrap': wrap}, globals=globals())
