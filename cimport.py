...
class GeographyElement(WKTElement):
    ...
    def __init__(self, *args, **kwargs):
        ...
        expression.Function.__init__(self, "ST_GeographyFromText",
                                     self.data)
...
def main():
  ...
  # of the shapefile.
  srid = 4326
  ne_file = '/data/natural-earth/ne_10m_admin_0_countries/' \
  ...
  xx = xx.assign(pd_area=xx.area) \
    .rename(columns=lambda s: s.lower()
            if isinstance(s, str) and s.isupper() else s)
  wba = pd.read_csv('/data/area/API_AG.LND.TOTL.K2_DS2_en_csv_v2_10181480.csv')
  wba = wba.loc[:, ['Country Code', '2017']]\
    .rename(columns={'Country Code': 'wb_api3c', '2017': 'wb_area'})
  xx = xx.merge(wba, on='wb_api3c', how='left')
  yy = xx.assign(geom=xx.geometry.apply(lambda x: from_text(x.wkt,
                                                            srid=4326))) \
         .drop('geometry', axis=1)

  engine = create_engine('postgresql://postgis:postgis@192.168.0.46:5432/groads')
  yy.to_sql('countries', engine, if_exists='replace',
            index=False,
