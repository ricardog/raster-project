...
@cli.command()
@click.argument(
    "what", type=click.Choice(["ab", "sr", "cs-ab", "cs-sr", "bii-ab", "bii-sr"])
)
...
def summary(what, scenario, years, npp, vsr, regions):      # noqa C901
    ...
    df_global = pd.DataFrame({"ID": [-1], "Name": ["Global"]}, columns=("ID", "Name"))
    vname = (
        "Abundance"
        if what == "ab"
        else "Richness"
    ...
    for year in years:
        ...
        if "Cells" not in df_global.columns:
            df_global["Cells"] = bii.shape[0] * bii.shape[1] - ma.count_masked(bii)
    ...
    df.to_csv(
        "%s-%s%s-%s-%04d-%04d.csv"
        % (scenario, vname, weight, area, years[0], years[-1]),
        index=False,
    )
    df_global.to_csv(
        "%s-%s%s-global-%04d-%04d.csv" % (scenario, vname, weight, years[0], years[-1]),
        index=False,
    )
...
def summarize(what, scenario, years, npp):
    ...
    for year in years:
        ...
        vnames = (
            ("Abundance", "CompSimAb") if what == "ab" else ("Richness", "CompSimSR")
        )
