...
def read_data(dirname, lu_name, scenario, year):
    ...
    for name in types:
        ...
        for intensity in lui.intensities():
            path = os.path.join(
                dirname, lu_name, "%s-%s_%s-%d.tif" % (scenario, name, intensity, year)
            )
...
def barplot(show):
    ...
    ax = plot_clustered_stacked(
        [df1, df2], ["1950", "2010"], title="Land use change 1950 to 2010"
    )
...
def timeline(scenario, years, output, model_dir, history, parallel):
    if parallel == 1:
        ...
        data = pool.map(
            unpack,
            itertools.izip(
                itertools.repeat(model_dir), itertools.repeat(scenario), years
            ),
        )
