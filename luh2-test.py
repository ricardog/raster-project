...
def project_year(model, model_dir, what, scenario, year):
    ...
    if what == "bii":
        vname = "bii"
        rsf[vname] = SimpleExpr(vname, "exp(%s) / exp(%f)" % (modf.output, intercept_f))
        rsn[vname] = SimpleExpr(vname, "exp(%s) / exp(%f)" % (modn.output, intercept_n))
...
def project(model, what, scenario, years, model_dir, parallel=1):
    ...
    if parallel == 1:
        tuple(map(lambda y: project_year(model, model_dir, what, scenario, y), years))
        return
