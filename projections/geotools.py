...
def srs_get_semi_major(srs):
    primem = srs.GetAttrValue("SPHEROID")
    if primem in ("WGS 84", "'WGS_1984'"):
        return osgeo.osr.SRS_WGS84_SEMIMAJOR
    raise RuntimeError("ricardog needs to deal with non-WGS84 reference systems")


def srs_get_inv_flattening(srs):
    primem = srs.GetAttrValue("SPHEROID")
    if primem in ("WGS 84", "'WGS_1984'"):
...
osgeo.osr.SpatialReference.GetPrimeMeridian = types.MethodType(
    srs_get_prime_meridian, osgeo.osr.SpatialReference
)
osgeo.osr.SpatialReference.GetSemiMajor = types.MethodType(
    srs_get_semi_major, osgeo.osr.SpatialReference
)
osgeo.osr.SpatialReference.GetInvFlattening = types.MethodType(
    srs_get_inv_flattening, osgeo.osr.SpatialReference
)
