...
def invertGeoTransform(geoTransform):
    ...
    outGeoTransform[3] = (
        -geoTransform[1] * geoTransform[3] + geoTransform[0] * geoTransform[4]
    ) * invDet
...
def createFields(inLayer, infos):
    ...
    for i in infos:
        if i.bands == 1:
            ...
            if fields_descript:
                fields_csv.write(i.fileBaseName + ";" + shortName + ";1" + "\n")
            if create_csv:
...
class fileInfo:
    def initFromFileName(self, fileName):
        ...
        self.nodata = [
            fh.GetRasterBand(i).GetNoDataValue() for i in range(1, self.bands + 1)
        ]
    ...
    for f in fileInfos:
        ...
        if f.bands == 1:
            ...
            while inFeat is not None:
                ...
                if gdalalloc:
                    value = os.popen(
                        "gdallocationinfo -valonly -wgs84 %s %s %s" % (f.fileName, x, y)
                    ).read()
                else:
                    value = band[rY, rX]
