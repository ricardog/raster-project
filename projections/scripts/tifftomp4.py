...
def get_stats(files):
    ...
    print(
        "min: %.2f / max: %.2f [%d x %d] : %d"
        % (min(low), max(high), x_size, y_size, sum(bands))
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Convert a series of raster " + "maps into a video sequence.",
    ...
    parser.add_argument(
        "-o", "--out", type=str, required=True, help="name of output file."
    )
    parser.add_argument("-t", "--title", default="", help="title for the video")
    parser.add_argument(
        "-p",
        "--palette",
        default="green",
    ...
    parser.add_argument(
        "-b", "--band", type=int, help="which band in input raster files to use"
    )
    parser.add_argument(
        "-f",
        "--files",
        ...
        nargs="+",
        required=True,
        help="GeoTIFF raster files to process",
    )
    parser.add_argument(
        "-s",
        "--stats",
