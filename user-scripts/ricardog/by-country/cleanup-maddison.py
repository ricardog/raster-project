...
def main(infile, outfile, start_year, long_form):
    ...
    csel = map(
        lambda cc: True if (isinstance(cc, str) and len(cc) == 2) else False, cfips
    )
    cint_name = tuple(filter(lambda cc: (isinstance(cc, str) and len(cc) == 2), cfips))
    start_row = xls.loc[
        xls["GDP per capita"] == start_year,
    ].index.values[0]
    ...
    if long_form:
        ...
        gdp = by_fips.melt(
            id_vars="fips", value_vars=years, var_name="year", value_name="GDP"
        ).dropna()
