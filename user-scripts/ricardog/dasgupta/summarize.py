...
def plot_all(data, indicator, npp):
    ...
    plot_a = set(
        tuple(filter(lambda c: re.match("early", c), data.Scenario)) + ("base",)
    )
    plot_b = set(
        tuple(filter(lambda c: re.match("late", c), data.Scenario)) + ("base", "early")
    )
    lines = (len(plot_a), len(plot_b))

    fig, axs = plt.subplots(1, 2, sharey=True)
    colors = ["windows blue", "amber", "dusty purple"]
    colors = ["windows blue", "amber", "faded green", "dusty purple", "scarlet"]
    colors = [
        "#a6cee3",
        "#1f78b4",
        "#b2df8a",
...
@click.option(
    "--indicator",
    "-i",
    type=click.Choice(("BIIAb", "Abundance", "CompSimAb", "Abundance-nf")),
    default="BIIAb",
)
@click.option(
    "--raster-dir",
...
def worldwide(npp, indicator, raster_dir):
    ...
    for path in filter(
        lambda p: fnmatch(p.name, f"dasgupta-*-{indicator}-20*.tif"),
        raster_dir.iterdir(),
    ):
        print(path)
