...
def pline(p, df, column, color="black", line_width=6):
    ...
    p.line("year", "data", source=src, line_width=line_width, color=color)
...
def indicators(merged, local, out):
    ...
    base_url = (
        "http://ipbes.s3.amazonaws.com/weighted/" + "%s-%s-%s-%%s-%%04d-%%04d.csv"
    )
    if local:
        ...
        for indicator in ("BIIAb", "BIISR"):
            ...
            if merged:
                if scenario == "historical":
                    ...
                    subset = pd.concat([hsubset[indicator], subset], ignore_index=True)
                    glob = pd.concat([hglob[indicator], glob], ignore_index=True)
...
def landuse():
    ...
    for scenario in filter(lambda s: s != "historical", storage.keys()):
        ...
        p = figure(x_range=(df.index[0], df.index[-1]), y_range=(0, 100), title=title)
        p.grid.minor_grid_line_color = "#eeeeee"

        p.patches(xs="year", ys="data", color="color", legend="label", source=source)
        ...
        p.add_tools(
            HoverTool(
                tooltips=[("Year", "$x{0}"), ("Percent", "$y"), ("Land use", "@label")]
            )
        )
        row.append(p)
        if len(row) == 2:
...
def deltas(local, out, subregions):
    ...
    for indicator in ("BIIAb", "BIISR"):
        ...
        r3 = plt.circle(
            ...
            legend="Scenario",
            size=8,
            fill_color=factor_cmap("Scenario", palette=Spectral6, factors=names),
        )
        plt.add_tools(
            HoverTool(
                renderers=[r3],
...
def violin(local, out):
    ...
    base_url = "http://ipbes.s3.amazonaws.com/summary/" + "%s-%s-%s-%%s-%%04d-%%04d.csv"
    if local:
        print("Using local summary files")
        base_url = "file://ipbes-weighted/%s-%s-%s-%%s-%%04d-%%04d.csv"
